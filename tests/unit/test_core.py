...
class TestDocSyncCore:
    ...
    async def test_sync_operation(
        self, temp_dir: Path, sample_config: dict, sample_files: dict
    ):
    ...
    def test_report_generation_invalid_template(
        self, temp_dir: Path, sample_config: dict
    ):
    ...
    def test_path_traversal_protection(
        self, temp_dir: Path, sample_config: dict, security_test_data: dict
    ):
...
class TestErrorHandling:
    ...
    def test_error_chaining(self):
        ...
        with pytest.raises(DocSyncError) as exc:
            raise DocSyncError("Wrapped error") from original_error

        assert exc.value.__cause__ is original_error
        assert "Original error" in str(exc.value.__cause__)
